...
class Authenticator extends Google2FA
{
    ...
    protected function verifyAndStoreOneTimePassword($one_time_password)
    {
        return $this->storeOldTimestamp(
            $this->verifyGoogle2FAMulti(
                $one_time_password
            )
        );
    }

    protected function verifyGoogle2FAMulti($one_time_password)
    {
        $mfa = $this->getUser()->authenticator;
        foreach ($mfa as $key) {
            $valid =  $this->verifyKey(
                $key->secret,
                $one_time_password,
                $this->getWindow(),
                null, // $timestamp
            ...
            if ($valid) {
                $key->last_used = Carbon::now();
                $key->last_ip = $this->request->ip();
                $key->save();
                return $valid;
            }
        }
    ...
    public function isActivated()
    {
         ...
         * 2 (H)    v       v       v
         */
        $policy = $this->getUser()->authenticator_policy_id;
